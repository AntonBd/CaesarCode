...
public class CaesarCode {
    ...
    public static void encrypt() {
        try(FileReader reader = new FileReader(fileFrom);
            ...
            while (reader.ready()) {
                char x = (char)reader.read();                          //Читаем символ из потока
                ...
                if (sec_position >= alphabet.length()) {               //Если позиция + ключ выходит за пределы длины алфавита
    ...
    public static void applyBruteForce() {
        ...
        for (int i = 0; i < alphabet.length(); i++) {
            ...
            //Цикл для декодирования текста
            //Цикл начинается с 1, т.к. в позиции 0 стоит служебный символ, который файл txt добавляет перед текстом
            //Если на иных компьютерах txt не добавляет служебных символов или добавляет больше, то j следует скоррект.
        ...
        //Определяем максимальный балл, набранный по логике ниже, и записываем в файл вариант с наибольшим баллом
        int max = 0;
